...
public function search_cases($data)
{
	...
	{
        $this->db->where('manage_cases.id',$title);
	}
	else if(!empty($department_id))
	{
        $this->db->where('manage_cases.department_id',$department_id);
	} 
	else if(!empty($case_category_id))
	{
        $this->db->where('manage_cases.case_cateid',$case_category_id);
	} 
	...
	{
        $this->db->where('manage_cases.court_id',$court_id);
	} 
	else if(!empty($year))
	{
        $this->db->where('manage_cases.year',$year);
	}
	
	if(!empty($start_date) && !empty($end_date))
	{
		$this->db->where('manage_cases.Filling_date >=',$start_date);
		$this->db->where('manage_cases.Filling_date <=',$end_date);
	}
